...
const extractTextFromPdf = async (file: File): Promise<{doses: string[], warnings: string[]}> => {
    ...
    type PageElement = 
        | { type: 'text', text: string, x: number, y: number, height: number } 
    ...
    for (let pageNum = 1; pageNum <= numPages; pageNum++) {
        try {
            ...
            Array.from(linesMap.entries())
                ...
                .forEach(([y, items]) => {
                    ...
                    if (text.trim().length > 0) {
                        pageElements.push({ type: 'text', text, x, y, height });
                    }
                });
            ...
            let currentParagraphLines: { text: string }[] = [];
            let lastLine: { y: number, height: number } | null = null;

            const finalizeParagraph = () => {
                if (currentParagraphLines.length === 0) return;
                
                const paragraphText = currentParagraphLines.reduce((acc, line) => {
            ...
            for (const element of pageElements) {
                if (element.type === 'text') {
                    const { text, x, y, height } = element;
                    
                    const isListItem = /^\s*([•●▪\u2022*-]|\d+\.|\w\))\s/.test(text.trim());
                    const isLikelyHeading = !isListItem && x < 80 && text.trim().split(' ').length < 15 && !/[.!?]$/.test(text.trim()) && text.trim().length > 3;
            ...
            finalizeParagraph(); // Final call for any remaining content
        } catch (pageError) {
